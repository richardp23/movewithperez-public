...
## Extensions (Quality Pass v1)

- **Fluid headline and spacing tokens**: `.display-xl` now uses a fluid clamp scale; `.title-xl` is used for non-home H1s; `.section-y` uses fluid vertical rhythm instead of fixed 4rem.
- **Wider responsive shell**: `.shell` replaces repeated page wrappers and scales from 72rem up to 92rem with breakpoints at 80rem, 96rem, and 120rem.
- **Interactive card language**: `.card-interactive` defines border/background/transform hover behavior with `--ease-soft`; linked cards may opt into `.card-media-zoom` for a subtle media scale effect.
- **Media fallback framing**: `.media-frame` applies a surface-card background with a hairline border so delayed/missing images read as intentional framed panels.
- **Editorial rails at xl+**: guide pages use a sticky facts rail (`.page-rail` + `.guide-aside`), insights articles use a sticky TOC rail (`.toc-layout` + `.toc-rail`), and process sections may pair timelines with a sticky talk-it-through rail.
- **Reveal rule remains unchanged**: reveal animations stay transform-only (`translateY`) with no opacity transitions to keep content visible during partial JS hydration.
